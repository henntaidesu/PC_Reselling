 ...
 * 全站图表的配色与通用轴 / 提示框配置。
 *
 * 放在 utils 而不是 Dashboard 目录下：汇率页那张折线图也用它。更要紧的是
 * EChart.vue 按需引入之后**不再挂 echarts 自带的 'dark' 主题**（那个主题文件是 UMD 的，
 * import 进来会把全量 echarts 又拉回去，白费按需引入），所以每张图的文字色、轴色、
 * 提示框底色都得自己给全——这里就是那一份「自己给全」的来源。
 * 新加图表时别再手写颜色，从这里取。
 *
