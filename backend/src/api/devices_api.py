...
@router.post("/{device_id}/pool-draw")
def confirm_pool_draw(device_id: int):
    """确认把这台设备的日元从资金池里扣掉：池内余额当场减少，成本改按注资批次的汇率算。

    单独一个接口而不是表单上的一个字段：详情页改一个字段就 PUT 一次，
    「动池子」这种事不该跟着防抖自动保存一起发生。
    """
    return _pool_draw(device_id, True)


@router.delete("/{device_id}/pool-draw")
def cancel_pool_draw(device_id: int):
    """撤销扣除：扣款行删掉，钱还回池子，成本退回按购入日牌价折算。"""
    return _pool_draw(device_id, False)


def _pool_draw(device_id: int, confirmed: bool):
    try:
        funds.set_confirmed("device", device_id, confirmed)
    except LookupError:
        raise HTTPException(status_code=404, detail="台设备不存在")
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    return _result(device_id, [])
