...
# 状态是**一条单线**：意向购入 → 已购入 → 待测试 → 测试通过 / 测试不通过 → 回国中
# → 转寄中 → 已签收 → 已打款。「已签收」「已打款」是中国买家的动作，走到「已打款」这笔生意才算完。
# 「测试不通过」是分叉终点，通常不会再往后走（卡有问题，退货或另行处理）。
#
# 「意向购入」是唯一一个**东西还不是自己的**状态：看上了、还没拍下/还没付钱，先建一行把
# 链接与心理价位记下来。它不参与任何金额口径上的特殊处理——成本、利润、资金池一律只看
# 金额字段填没填，所以给意向行填了价，池子就会把这笔钱算成已花出去；要预留预算是想要的
# 行为就填，不想占用就别填价。新建的草稿默认仍是「已购入」（绝大多数记录是买完才录的）。
